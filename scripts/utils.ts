...
const getTree = (log?: boolean) => {
    ...
    const whitelistAddresses: string[] = (
        readFileSync('whitelist.csv', { encoding: 'utf-8' }) as string
    ).split('\n');
    if (log)
        console.log(
            `Generating a tree from ${whitelistAddresses.length} whitelisted addresses...`,
        );
    ...
    if (log)
        console.log(
            ...
            } addresses with key ${key} and tree root ${tree.getHexRoot()}`,
        );
...
function getPassword(_address: string) {
    const passwords = JSON.parse(
        readFileSync('passwords.json', { encoding: 'utf-8' }),
    ) as {
        password: string;
        proof: string;
        address: string;
    ...
    return (
        passwords.find(({ address }) => address == _address)?.password ??
        'invalid address'
    );
}
