...
export const setTrustedRemote__task = async (
    ...
    const dstContract: string = taskArgs.dstcontract ?? getOtherChainDeployment(taskArgs.dstchain).address;
    await pearlClubONFT.setTrustedRemoteAddress(dstChainID, dstContract).then(
        (tx) => console.log(`Set ${dstContract} on ${taskArgs.dstchain} with LZID of ${dstChainID} as trusted remote successfully \n txID: ${tx.hash}`)
    );
}